...
struct ShotConfigScreen: View {
    // 1. Añadimos el entorno de descarte para cerrar la vista actual correctamente
    ...
    // Acción mejorada para guardar y cerrar solo esta vista
    private func saveConfigAndReturn() {
        ...
        let sh = String(format: "[SH%02d%02d%02d%02d%02d]", a, a, c, d, e)
        
        communicator.sendCommand(sh)
        onSaveConfig(shotConfig)
        
        // Cierra la vista actual para volver a la pantalla de Drill
        dismiss()
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                // Sliders
                VStack(spacing: 20) {
                    ModernSlider(label: "POWER_LEVEL", value: $speedTemp, range: 0...255, icon: "bolt.fill") {
                        let v = Int(speedTemp.rounded())
                        communicator.sendCommand(String(format: "[A%03d]", v))
                        communicator.sendCommand(String(format: "[B%03d]", v))
                        triggerDetectionGlitch()
                    }
                    ModernSlider(label: "INTERVAL_DELAY", value: $delayTemp, range: 0...255, icon: "timer") {
            ...
            if showAbortConfirmation {
                ...
                VStack(spacing: 20) {
                    ...
                    VStack(spacing: 12) {
                        Button(action: saveConfigAndReturn) {
                            Text("SAVE_AND_EXIT").font(.system(size: 14, weight: .bold, design: .monospaced))
                        ...
                        Button(action: {
                            onCancel()
                            dismiss() // Descarta y vuelve al Drill
                        }) {
                            Text("DISCARD_ALL").font(.system(size: 14, weight: .bold, design: .monospaced))
                                .frame(maxWidth: .infinity).padding()
                        ...
                        Button(action: { showAbortConfirmation = false }) {
                            Text("RETURN_TO_EDITOR").font(.system(size: 12, weight: .bold, design: .monospaced))
...
struct YOLOTargetBox: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            ZStack { cornerShape(rotation: 0); cornerShape(rotation: 90); cornerShape(rotation: 180); cornerShape(rotation: 270) }.foregroundColor(.yellow)
    ...
    @ViewBuilder private func cornerShape(rotation: Double) -> some View {
        GeometryReader { geo in
            Path { p in p.move(to: CGPoint(x: 0, y: 8)); p.addLine(to: CGPoint(x: 0, y: 0)); p.addLine(to: CGPoint(x: 8, y: 0)) }
...
struct TennisCourtShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path(); path.move(to: CGPoint(x: rect.width * 0.15, y: 0)); path.addLine(to: CGPoint(x: rect.width * 0.85, y: 0))
        path.addLine(to: CGPoint(x: rect.width, y: rect.height)); path.addLine(to: CGPoint(x: 0, y: rect.height)); path.closeSubpath(); return path
    }
}

struct TennisCourtLines: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path(); path.move(to: CGPoint(x: rect.midX, y: 0)); path.addLine(to: CGPoint(x: rect.midX, y: rect.height))
        path.move(to: CGPoint(x: rect.width * 0.07, y: rect.height * 0.33)); path.addLine(to: CGPoint(x: rect.width * 0.93, y: rect.height * 0.33))
        path.move(to: CGPoint(x: rect.width * 0.03, y: rect.height * 0.66)); path.addLine(to: CGPoint(x: rect.width * 0.97, y: rect.height * 0.66)); return path
    }
}
...
struct ModernSlider: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack { Image(systemName: icon).foregroundColor(.dragonBotPrimary); Text(label); Spacer(); Text("\(Int(value))").foregroundColor(.dragonBotPrimary) }
            .font(.system(size: 10, weight: .bold, design: .monospaced)).foregroundColor(.white.opacity(0.8))
            Slider(value: $value, in: range, step: 1) { editing in if !editing { onFinished() } }.accentColor(.dragonBotPrimary)
