...
// MARK: - Soporte para Gesto de Retroceso (Indispensable)
// Esta extensión permite que el gesto de "swipe" funcione aunque ocultemos el botón físico
    ...
    struct SwapConfigScreen: View {
        ...
        var body: some View {
            ZStack {
                ...
                VStack(spacing: 0) {
                    // --- Indicador de Conexión Minimalista (Debajo de Nav Bar) ---
                    HStack {
                        Circle()
            ...
            // --- Implementación de Navegación Intuitiva ---
            .navigationTitle("MODO SWAP")
            .navigationBarTitleDisplayMode(.inline)
            .navigationBarBackButtonHidden(true) // Oculta el botón "< Atrás"
        ...
        private func editorModal(id: Int) -> some View {
            ZStack {
                Color.black.opacity(0.9).ignoresSafeArea()
                    .onTapGesture { withAnimation { editingSwap = nil } }
                ...
                GeometryReader { geo in
                    ...
                    .gesture(
                        DragGesture(minimumDistance: 0)
                            .onChanged { value in
                                tempConfig.x = Double((max(0, min(geo.size.width, value.location.x)) / geo.size.width) * 255.0)
                                tempConfig.y = Double((1.0 - (max(0, min(geo.size.height, value.location.y)) / geo.size.height)) * 255.0)
                            }
                    )
                }
            }
        ...
        private func editorHeader(id: Int) -> some View {
            HStack {
                ...
                Spacer()
                Button(action: { withAnimation { editingSwap = nil } }) {
        ...
        private func openEditor(id: Int) { tempConfig = (id == 1) ? channel1 : channel2; withAnimation { editingSwap = id } }
        private func saveChanges(id: Int) { if id == 1 { channel1 = tempConfig } else { channel2 = tempConfig }; withAnimation { editingSwap = nil } }
